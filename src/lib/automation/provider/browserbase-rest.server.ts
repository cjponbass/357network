...
type Config = { apiKey: string; projectId: string; modelApiKey: string; modelName: string; submitEnabled: boolean };
...
export function readBrowserbaseRestConfig(): Config {
  ...
  if (!apiKey || !projectId || !modelApiKey) throw new Error("Browserbase REST provider configuration is incomplete.");
  return { apiKey, projectId, modelApiKey, modelName: process.env["STAGEHAND_MODEL"]?.trim() || "openai/gpt-5.4-mini", submitEnabled: process.env["AUTOMATION_ENABLE_SUBMIT"] === "true" };
}
...
export function createBrowserbaseRestProvider(owner: ProviderOwner, config = readBrowserbaseRestConfig()): BrowserAutomationProvider {
  ...
  return {
    ...
    async submit(session): Promise<ProviderSubmitResult> {
      ...
      const observed = await request<{ selector?: string }[]>(config, `/sessions/${encodeURIComponent(session.sessionId)}/observe`, { instruction: "Identify only the final employer application submit button. Do not click it." });
      const selector = observed.find((item) => item.selector)?.selector;
      if (!selector) return { submitted: false, blockers: [block("unsupported_widget", "Final submit control was not unambiguously identified.")] };
      await act(config, session.sessionId, selector, "click", []); current.submitted = true; return { submitted: true, blockers: [] };
    },
    ...
    async captureEvidence(session): Promise<ProviderEvidence> { const current = state(session); return { screenshotPath: null, pageUrl: current.pageUrl, capturedAt: new Date().toISOString(), browserbaseSessionId: session.sessionId }; },
...
async function request<T=Record<string,unknown>>(config:Config,path:string,body:unknown):Promise<T>{ const response=await fetch(`${STAGEHAND}${path}`,{method:"POST",headers:{"content-type":"application/json","x-bb-api-key":config.apiKey,"x-bb-project-id":config.projectId,"x-model-api-key":config.modelApiKey},body:JSON.stringify(body)}); const text=await response.text(); if(!response.ok) throw new Error(`Browserbase Stagehand failed (${response.status}).`); const parsed=text?JSON.parse(text) as {success?:boolean;data?:T}:{}; if(parsed.success===false) throw new Error("Browserbase Stagehand operation failed."); return (parsed.data??parsed) as T; }
