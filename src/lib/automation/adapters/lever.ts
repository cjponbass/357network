/**
 * Lever adapter — standard Lever application fields plus live-form compatibility.
 * Employer-specific questions are supplied by the browser provider at runtime.
 */
...
export const leverAdapter: AtsAdapter = {
  ...
  inspectForm(targetUrl: string): AdapterInspectResult {
    const notes = [
      "Field list represents Lever's common application fields. Employer-specific questions are discovered from the live form by the configured browser provider.",
    ];
    if (!/lever\.co|jobs\.lever/.test(targetUrl)) {
      notes.push("URL does not look like a Lever job page; treat this static field list as indicative only.");
    }
    return { fields: LEVER_FIELDS, notes };
  },
