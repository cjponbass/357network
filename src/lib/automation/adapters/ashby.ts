/** Ashby adapter — common fields plus authoritative live-form mapping. */
...
export const ashbyAdapter:AtsAdapter={
  provider:"ashby",displayName:"Ashby",implemented:true,
  inspectForm(targetUrl:string):AdapterInspectResult{const notes=["Field list represents Ashby's common application fields. Employer-specific questions are discovered from the live form."];if(!/ashbyhq\.com/.test(targetUrl))notes.push("URL does not look like an Ashby job page; treat this static field list as indicative only.");return{fields:ASHBY_FIELDS,notes};},
