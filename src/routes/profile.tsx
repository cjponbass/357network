...
type ExtendedProfile = {
  ...
  is_mason: boolean; employer_discoverable: boolean;
};
type FormState = {
...
const EMPTY: FormState = {
  ...
  linkedin_url: "", github_url: "", website_url: "", is_mason: false, employer_discoverable: false,
};
...
function ProfilePage() {
  ...
  return <main style={pageStyle}><Nav/><h1 style={{fontSize:34,marginBottom:8}}>Candidate Profile</h1><p style={{color:"#4b5563"}}>Keep reusable, user-owned facts in one private profile. 357Network never invents missing application answers.</p><form onSubmit={save} style={panelStyle}>
    ...
    <h2 style={sectionHeading}>Masonic & employer visibility</h2>
    <label style={checkRow}><input type="checkbox" checked={form.is_mason} onChange={e=>setForm(prev=>({...prev,is_mason:e.target.checked}))}/><span><strong>I am a Mason</strong><small style={hintStyle}>This status is only shown to employers if you also opt into employer discoverability.</small></span></label>
    <label style={checkRow}><input type="checkbox" checked={form.employer_discoverable} onChange={e=>setForm(prev=>({...prev,employer_discoverable:e.target.checked}))}/><span><strong>Allow prospective employers on 357Network to find my profile</strong><small style={hintStyle}>Default is private. Only recruiting-safe professional fields are searchable; your street address, phone, email, documents, saved answers and application history stay private.</small></span></label>
...
async function syncStructuredAnswers(userId:string,form:FormState):Promise<string|null>{
  const canonical=[["Street address",form.address_line1],["Address line 2",form.address_line2],["City",form.city],["State province region",form.region],["Postal ZIP code",form.postal_code],["Country",form.country]] as const;
...
function Field({id,label,value,type="text",set}:{id:keyof FormState;label:string;value:string;type?:string;set:(id:keyof FormState,value:string)=>void}){return <label style={{display:"grid",gap:6}}><span style={{fontSize:14,fontWeight:600}}>{label}</span><input id={String(id)} type={type} value={value} onChange={e=>set(id,e.target.value)} style={inputStyle}/></label>;}
function Area({id,label,value,set,hint}:{id:keyof FormState;label:string;value:string;set:(id:keyof FormState,value:string)=>void;hint?:string}){return <label style={{display:"grid",gap:6}}><span style={{fontSize:14,fontWeight:600}}>{label}</span>{hint?<span style={{fontSize:13,color:"#6b7280"}}>{hint}</span>:null}<textarea id={String(id)} value={value} onChange={e=>set(id,e.target.value)} rows={4} style={{...inputStyle,resize:"vertical"}}/></label>;}
function Nav(){return <nav style={{display:"flex",flexWrap:"wrap",gap:14,marginBottom:28}}><a href="/dashboard" style={navLink}>Dashboard</a><a href="/discover" style={navLink}>Discover</a><a href="/jobs" style={navLink}>Jobs</a><a href="/applications" style={navLink}>Applications</a><a href="/documents" style={navLink}>Documents</a><a href="/billing" style={navLink}>Billing</a><a href="/settings" style={navLink}>Settings</a></nav>;}
const pageStyle:React.CSSProperties={maxWidth:980,margin:"0 auto",padding:"40px 24px",fontFamily:"system-ui"}; const navLink:React.CSSProperties={color:"#1d4ed8",textDecoration:"none"}; const panelStyle:React.CSSProperties={display:"grid",gap:18,border:"1px solid #e5e7eb",borderRadius:12,padding:20,margin:"28px 0"}; const gridStyle:React.CSSProperties={display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(240px,1fr))",gap:14}; const inputStyle:React.CSSProperties={border:"1px solid #d1d5db",borderRadius:8,padding:"11px 12px",fontSize:16}; const primaryButton:React.CSSProperties={border:0,borderRadius:8,padding:"11px 14px",background:"#111827",color:"white",cursor:"pointer",width:"fit-content"}; const sectionHeading:React.CSSProperties={fontSize:19,margin:"8px 0 0"}; const checkRow:React.CSSProperties={display:"flex",gap:10,alignItems:"flex-start",border:"1px solid #e5e7eb",borderRadius:10,padding:14}; const hintStyle:React.CSSProperties={display:"block",color:"#6b7280",marginTop:4,lineHeight:1.4};
