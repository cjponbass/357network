...
function ApplicationDetailPage() {
  ...
  const load = useCallback(async () => {
    if (!user) return;
    setError(null);
    const appResult = await supabase.from("applications").select("*").eq("id", applicationId).eq("user_id", user.id).maybeSingle();
    if (appResult.error) {
  ...
  const selectedResume = useMemo(() => documents.find((doc) => doc.id === resumeId) ?? null, [documents, resumeId]);
  const selectedCover = useMemo(() => documents.find((doc) => doc.id === coverId) ?? null, [documents, coverId]);
  ...
  async function saveApplication(event: React.FormEvent) {
    ...
    const submittedAt = status === "submitted" && !application.submitted_at ? new Date().toISOString() : application.submitted_at;
  ...
  if (!application) {
    return <main style={pageStyle}><a href="/applications" style={navLink}>← Applications</a><h1>Application not found</h1><p style={{ color: "#6b7280" }}>This application does not exist or is not owned by your account.</p></main>;
  }

  return (
    <main style={pageStyle}>
      <nav style={{ display: "flex", gap: 14, marginBottom: 26 }}>
        <a href="/applications" style={navLink}>← Applications</a><a href="/dashboard" style={navLink}>Dashboard</a><a href="/documents" style={navLink}>Documents</a>
      </nav>
      <h1 style={{ fontSize: 34, marginBottom: 6 }}>{job?.title ?? "Application"}</h1>
      <p style={{ color: "#4b5563", marginTop: 0 }}>{job?.company ?? "Saved job"}</p>
      <p style={{ color: "#6b7280", fontSize: 14 }}>ATS: {job?.ats_name ?? "Not detected yet"} · Created {new Date(application.created_at).toLocaleString()}</p>
      {error ? <p role="alert" style={{ color: "#b91c1c" }}>{error}</p> : null}

      <form onSubmit={saveApplication} style={panelStyle}>
        <h2 style={sectionTitle}>Application details</h2>
        <label style={labelStyle}>Status<select value={status} onChange={(event) => setStatus(event.target.value as ApplicationStatus)} style={inputStyle}>{APPLICATION_STATUSES.map((value) => <option key={value} value={value}>{APPLICATION_STATUS_LABELS[value]}</option>)}</select></label>
        <label style={labelStyle}>Resume<select value={resumeId} onChange={(event) => setResumeId(event.target.value)} style={inputStyle}><option value={NONE}>None selected</option>{documents.filter((doc) => doc.kind === "resume" || doc.kind === "other").map((doc) => <option key={doc.id} value={doc.id}>{doc.name} · {DOCUMENT_KIND_LABELS[doc.kind]}</option>)}</select></label>
        <label style={labelStyle}>Cover letter<select value={coverId} onChange={(event) => setCoverId(event.target.value)} style={inputStyle}><option value={NONE}>None selected</option>{documents.filter((doc) => doc.kind === "cover_letter" || doc.kind === "other").map((doc) => <option key={doc.id} value={doc.id}>{doc.name} · {DOCUMENT_KIND_LABELS[doc.kind]}</option>)}</select></label>
      ...
      <section style={panelStyle}>
        ...
        <p style={{ marginBottom: 0, color: "#6b7280", fontSize: 14 }}>Automated submission remains server-controlled. CAPTCHA, login walls and unresolved required questions stop the run; success is recorded only after confirmation evidence is verified.</p>
      </section>
